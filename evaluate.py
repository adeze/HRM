...
from utils.device import get_device, choose_dist_backend
...
def launch():
    ...
    try:
        train_state.model.load_state_dict(torch.load(eval_cfg.checkpoint, map_location=str(DEVICE)), assign=True)
    except:
        train_state.model.load_state_dict({k.removeprefix("_orig_mod."): v for k, v in torch.load(eval_cfg.checkpoint, map_location=str(DEVICE)).items()}, assign=True)
