...
from utils.device import get_device, device_str, choose_dist_backend
...
def launch():
    ...
    if "LOCAL_RANK" in os.environ:
        # Initialize distributed with appropriate backend
        backend = choose_dist_backend()
    ...
    try:
        train_state.model.load_state_dict(torch.load(eval_cfg.checkpoint, map_location=device_str(DEVICE)), assign=True)
    except:
        train_state.model.load_state_dict({k.removeprefix("_orig_mod."): v for k, v in torch.load(eval_cfg.checkpoint, map_location=device_str(DEVICE)).items()}, assign=True)
